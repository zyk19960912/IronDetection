...
class DataProcessor(object):
    ...
    def process_data(self, train_file, test_file, load_saved_data=True):
        ...
        self.embedding_model = spacy.load("en")
        ...
        self.pos_tags = copy.deepcopy(pos_tags)
        self.text_data = copy.deepcopy(text_data)
        
#         print(str(len(features))+","+str(len(features[0])))
        print("Saved data!")
        return train_data, test_data
    
    def getFeature(self, input_data):
        print("Starting to get features!")

        features = []
        self.embedding_model = spacy.load("en")
        input_data = self.normalise_tweet(input_data)

        features = self.process_a_tweet(input_data)
        self.text_data.append(input_data)
        self.pos_tags.append(' '.join(self.extract_pos_tags(input_data)))
        print(len(features))
        ...
        ngram_vectorizer = TfidfVectorizer(analyzer='char_wb', ngram_range=(1, 3), \
                                           max_features=self.n_features, norm='l2')
        counts = ngram_vectorizer.fit_transform(self.text_data)
        n_grams_features = counts.toarray()
        features = np.append(features, n_grams_features[-1])
        del n_grams_features
        print(len(features))
        ...
        tfidf_vectorizer = TfidfVectorizer(ngram_range=(1, 3), lowercase=False, \
                                           max_features=self.n_features, norm='l2')
        tfidfs = tfidf_vectorizer.fit_transform(self.text_data)
        tfidfs_features = tfidfs.toarray()
        features = np.append(features, tfidfs_features[-1])
        print(len(features))
        del tfidfs_features
